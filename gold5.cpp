...
#include <conio.h> // For clearscreen on windows

// Clear screen function this should be portable since it will do all modern OSs
void Clear()
{
#if defined _WIN32
    system("cls");
...
// Now write what DM will do if our check passes.
// In this case it doesnt matter how much gold we have. DM will make it more.
// We will use the '+' modifier to increment the amount by 2 so it is over the original value if you get a success roll.
} 

  if (charisma >= dc) {
   std::cout << "\n";
   std::cout << "DM (system call): You don't have enough gold; It's worth : " << gold+2 << "\n";
...
// We want to ask this before int main() because we want the condition to be "true" so it will jump us back to the beginning.
bool getRepeat()
{
...
int main()
{
   ...
   while (repeat)
   {
    ...
    // Since we are repeating, tell it to call the clear screen, THEN call the main program.
     Clear();
     shop();
     repeat = getRepeat();
